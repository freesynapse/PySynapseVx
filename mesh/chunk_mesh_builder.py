...
# convert data to numba-recognizable format
@njit
...
# add vertices to the vertex_data array, keeping track of the current index
@njit
...
# returns the world chunk index of a voxel
@njit
def get_chunk_index(world_vx_pos):
    wx, wy, wz = world_vx_pos
    cx = wx // CHUNK_SIZE
    cy = wy // CHUNK_SIZE
    cz = wz // CHUNK_SIZE
...
# checks if a neighbouring voxel is void, for hidden face removal
@njit
def is_void(local_vx_pos, world_vx_pos, world_voxels):
    chunk_index = get_chunk_index(world_vx_pos)
    if chunk_index == -1:
        return False
...
# computes the ambient occlusion of a voxel depending on its neighbours
@njit
...
# builds the chunk mesh, using hidden face removal
@njit
